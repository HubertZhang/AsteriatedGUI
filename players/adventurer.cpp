    ...
    CardAndSkill(paintStruct,parent)
{
    ...
    for(int i = 0;i < 5;i++)
    {
        disconnect(cheat->number[i],SIGNAL(beChecked()),cheat->ensure,SLOT(setCheckableTrue()));
        connect(cheat->number[i],SIGNAL(beChecked()),this,SLOT(cheatSet()));
        connect(cheat->number[i],SIGNAL(unChecked()),this,SLOT(cheatReset()));
    }
